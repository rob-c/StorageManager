...
public static class TerminalApp
{
    public static int Run()
    {
        ...
        Environment.SetEnvironmentVariable("STORAGEMANAGER_ASKPASS_UI", "tui");

        AnsiConsole.Write(new FigletText("Storage Manager").Color(Color.Teal));
    ...
    private static void RunStatus()
    {
        ...
        var report = service.GatherAsync(new StorageManager.Status.StatusRequest(host, user, paths))
                            .GetAwaiter().GetResult();
    ...
    private static void RunVsCode()
    {
        ...
        var target = new StorageManager.VsCode.VsCodeTarget(
            alias, host, user, string.IsNullOrWhiteSpace(jump) ? null : jump);
        var setup = new StorageManager.VsCode.VsCodeSetup(new StorageManager.VsCode.VsCodeCli());
        var sshConfig = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".ssh", "config");
        var settings = StorageManager.VsCode.VsCodeSettings.DefaultPath;

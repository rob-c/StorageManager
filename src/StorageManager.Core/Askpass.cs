...
{
    public const string ModeVariable = "STORAGEMANAGER_ASKPASS_MODE";
    public const string PasswordVariable = "STORAGEMANAGER_ASKPASS_PASSWORD";
    ...
    public static int Run(string prompt)
    {
        ...
        if (password is not null && isLoginPasswordPrompt)
        {
            ...
            // Never log anything derived from the secret (length/hash/fingerprint can
            // help an offline guesser). Record only that a password prompt was answered.
    ...
    /// <summary>With STORAGEMANAGER_DEBUG set, appends askpass activity to storagemanager-askpass.log in the temp directory.</summary>
    private static void Log(string message)
    {
        if (Environment.GetEnvironmentVariable("STORAGEMANAGER_DEBUG") is null)
            return;
        DebugLog(message);
    }

    /// <summary>Unconditionally appends to storagemanager-askpass.log in the temp directory.</summary>
    public static void DebugLog(string message)
    {
        try
        {
            File.AppendAllText(Path.Combine(Path.GetTempPath(), "storagemanager-askpass.log"),

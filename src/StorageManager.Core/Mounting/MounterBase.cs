...
public abstract class MounterBase(Config config) : IMounter
{
    ...
    [
        ...
        // A jump is a double hop and gateways often do a slow reverse-DNS lookup
        // (~10s each) before responding, so give the banner exchange plenty of room.
        "-o", $"ConnectTimeout={(Config.JumpHost is not null ? 60 : 10)}",
